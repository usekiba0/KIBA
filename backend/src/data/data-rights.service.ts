...
export class DataRightsService {
  ...
  async exportUserData(userId: string) {
    ...
    return { user, subscription, messages, nutritional_analyses: analyses, session_summaries: summaries, crisis_alerts: alerts, sessions };
  }

  async deleteUserData(userId: string): Promise<void> {
    const sub = await this.subRepo.findOne({ where: { user_id: userId } });
    if (sub?.stripe_subscription_id) {
      try { await this.stripeService.cancelSubscription(sub.stripe_subscription_id); } catch {}
    }
