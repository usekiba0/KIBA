...
async function main() {
  ...
  const dupGroups = await pg.query(`
    SELECT user_id, recurrence_local_time, count(DISTINCT recurrence_parent_id) AS chains
    FROM scheduled_reminders
    WHERE status='pending' AND recurrence_rule='daily'
    GROUP BY user_id, recurrence_local_time
  ...
  for (const g of dupGroups.rows) {
    // All pending rows in this (user, time) slot, oldest chain first — keep the
    // first parent, cancel every other chain's pending row(s).
    ...
    `, [g.user_id, g.recurrence_local_time])).rows;
    ...
    for (const r of rows) {
      ...
      if (keep) continue;
      if (LIVE) {
        await pg.query(`UPDATE scheduled_reminders SET status='cancelled' WHERE id=$1`, [r.id]);
        if (r.bull_job_id) {
